...
- [x] Validate SSO token + partner auth against hackathon SSO with dashboard credentials — **live PASS end-to-end (2026-07-22, reconfirmed):** `exchangeToken ok` + `authenticatePartner ok` in one smoke run. Partner field must be literal `HACKATHON_SSO` (not unresolved `{{partner_code}}`); paste exchange code from page text, not screenshots. Codes are single-use — each re-smoke needs a fresh mint.
...
- [x] eGov AI token + one assistant call; decide composition vs local `LlmPort` — **live PASS (2026-07-22):** `token ok (credits_remaining=200)` then `ai_assistant generate ok` via `pnpm smoke:platform -- --only=egov-ai --write` (`chars=185`, `session_id` returned). Smoke now calls generate only behind `--write` (spends credits); safe default remains token-only. Composition decision: `EgovAiPort` used directly for BANGON narration (`explainEligibility`), `LlmPort`/Ollama kept separate for orchestrator agents — not unified
- [x] eGovPay generate/get/void with HMAC digest verification — **live PASS (2026-07-22):** `generatePayment ok` via `pnpm smoke:platform -- --only=egov-pay --write`. Real bug found and fixed: the platform strips a `test_` prefix from the API token before using it as the HMAC digest key — the docs say the digest is "keyed by the API token" with no mention of stripping, but the raw prefixed token returns 422 `"The digest is not valid."` even with an otherwise-correct request (reproduced independently via the dashboard's own "Test" tab with a manually-verified digest, ruling out a transport/config issue). Fixed in `eGovPayDigestKeyFromToken` (`packages/adapters-egov-platform/src/egov-pay.ts`), unit-tested in `egov-pay.test.ts`.
- [x] eGovChain `eth_blockNumber` smoke via thin JSON-RPC port — **PASS**: `eth_blockNumber ok`
- [~] eReport datasets + submit/OTP path — **live PASS (read path, 2026-07-22):** `token + getReportTypes ok (12)` and `getRegions ok (18)`. `submitComplaint`/OTP remain **opt-in** (`--write` + `SMOKE_EREPORT_SUBMIT=1` + location/mobile/email env) — filing creates a real case; deliberately not default for hackathon. BANGON `reportBenefitNonDelivery` can call submit when demoing; smoke stays safe unless you opt in.
