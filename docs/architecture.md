...
- **Application** (`packages/application/src/ports/index.ts`): `BenefitCatalogPort`, `BenefitMatchRepository`, `BenefitNotificationRepository`, `HashPort`. (`packages/application/src/use-cases/bangon.ts`): `findEligibleBenefits` (fund-check-then-match via `isFundedFromDbmResult` on SAAODB cascade / record totals; fails closed on fund-check errors), `notifyEligibility` (categorized eMessage: announcement, qualification, requirements, status, reminder), `disburseBenefit` (eGovPay; URLs from body or `EGOVPAY_*_URL`), `confirmCitizenIdentity` (Face Liveness API gate + eVerify Tier `face_liveness_session_id` + demographics), `anchorBenefitMatch` (local SHA-256; optional chain submit only when `EGOVCHAIN_ANCHOR_METHOD` is set), `explainEligibility` (eGov AI, post-decision only), `reportBenefitNonDelivery` (citizen-initiated eReport). Notification delivery is idempotent per recipient/category/context, uses a six-hour same-category cooldown and five-message daily cap, and persists only digests plus delivery metadata.
- **HTTP** (`packages/adapters-http` + `apps/api`): `createFaceLivenessHttpHandlers` + `createBangonHttpHandlers` mounted at `/bangon/liveness/session`, `/bangon/liveness/result/:token`, `/bangon/confirm-identity`, `/bangon/matches`, `/bangon/matches/:id/{notify,disburse,anchor,explain}`, `/bangon/report-non-delivery`.
- **Adapter** (`packages/adapters-persistence/src/index.ts`): `createInMemoryBenefitCatalog` (3 seed benefits), `createInMemoryBenefitMatchRepository`, `createInMemoryBenefitNotificationRepository`, `createNodeHashAdapter`. The in-memory notification ledger resets with the process; production must use durable storage and an atomic unique constraint for recipient/context digests.
