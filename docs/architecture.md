...
- **Application** (`packages/application/src/ports/index.ts`): `BenefitCatalogPort`, `BenefitMatchRepository`, `HashPort`. (`packages/application/src/use-cases/bangon.ts`): `findEligibleBenefits` (fund-check-then-match via `isFundedFromDbmResult` on SAAODB cascade / record totals; fails closed on transport errors), `notifyEligibility` (eMessage), `disburseBenefit` (eGovPay; URLs from body or `EGOVPAY_*_URL`), `confirmCitizenIdentity` (Face Liveness API gate + eVerify Tier `face_liveness_session_id` + demographics), `anchorBenefitMatch` (local SHA-256; optional chain submit only when `EGOVCHAIN_ANCHOR_METHOD` is set), `explainEligibility` (eGov AI, post-decision only), `reportBenefitNonDelivery` (citizen-initiated eReport).
